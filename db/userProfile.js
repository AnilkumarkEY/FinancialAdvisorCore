...
const insertEventTransaction = async (data) => {
  ...
  try {
    ...
    const allowedSourceResult = await client.query(allowedSourceQuery,[idevent_defination]);

    console.log(allowedSourceResult.rows,entityUrcAuthResult.rows);
    idevent_def_allowed_source = allowedSourceResult.rows[0].idevent_def_allowed_source;
